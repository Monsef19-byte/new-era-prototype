# -*- coding: utf-8 -*-
"""
_common.py — shared helpers for the online admin API (Vercel Python
serverless functions). Pure standard library, no third-party dependencies,
same philosophy as the local admin/server.py.

Storage model:
  - Content JSON (villas, home, apropos, opportunites, settings, blog, liens,
    admin_auth) lives in Vercel Blob under "content/<name>.json". If a section
    was never saved online yet, we fall back to the copy baked into this
    function's bundle at build time (_seed/<name>.json — a snapshot of
    admin/content/<name>.json at deploy time), so the online admin starts
    from the exact same data as the local one instead of showing empty pages.
  - Uploaded images/videos live in Vercel Blob under "assets/<filename>".
    generator.py downloads everything under "assets/" into homepage/assets/
    at build time, so published pages reference them exactly like today
    (src="assets/<filename>") with zero template changes.

Auth model: stateless signed cookie (no server-side session state, since
serverless functions don't share memory between invocations). The cookie
value is "<expiry-unix-ts>.<hmac-sha256 hex>", signed with ADMIN_SESSION_SECRET.
"""
import os
import re
import json
