...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        ...
        if not hook:
            return send_json(self, {"ok": False, "error": "DEPLOY_HOOK_URL non configuré"}, 500)
        try:
            req = urllib.request.Request(hook, method="POST")
            urllib.request.urlopen(req, timeout=15)
        except Exception as e:
            return send_json(self, {"ok": False, "error": "Échec du déclenchement du déploiement : {}".format(e)}, 500)
